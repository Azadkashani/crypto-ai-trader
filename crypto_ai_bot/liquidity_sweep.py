...
class LiquiditySweep:
    ...
    def detect(df, market_structure=None):
        # استفاده از Swing های موجود در market_structure
        ...
        # Sweep صعودی: قیمت از بالاترین High اخیر بالاتر رفته ولی close زیر آن برگشته
        buy_sweep = False
        sell_sweep = False
        if swings_high:
            last_high = swings_high[-1]["price"]
            if df["high"].iloc[-1] > last_high and last_close < last_high:
                sell_sweep = True  # جاروب نقدینگی فروش (Sell Side Liquidity)
        if swings_low:
            last_low = swings_low[-1]["price"]
            if df["low"].iloc[-1] < last_low and last_close > last_low:
                buy_sweep = True   # جاروب نقدینگی خرید (Buy Side Liquidity)
        return {
